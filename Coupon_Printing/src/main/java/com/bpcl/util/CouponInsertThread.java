...
public class CouponInsertThread extends Thread {
	...
	public CouponInsertThread(Integer quantity, CouponInspectionsService couponInspectionsService,

			CouponProductionService couponProductionService, LineInformation lineInformation, String couponProdUuid,
			PrinterStateService printerStateService, String convertCouponBarcodeString, String couponSize) {
		super();
		this.quantity = quantity;
		this.couponInspectionsService = couponInspectionsService;
		this.couponInspectionsService = couponInspectionsService;
		this.printerStateService = printerStateService;
		this.couponProdUuid = couponProdUuid;

		this.couponSize = couponSize;
		this.couponProductionService = couponProductionService;
		this.lineInformation = lineInformation;
		this.convertCouponBarcodeString = convertCouponBarcodeString;
	}

	public CouponInsertThread(Integer quantity2, CouponInspectionsService couponInspectionsService2,
			CouponProductionService couponProductionService2, LineInformationService lineInformationService) {
	...
	public void insertIntoDbtThread() throws IOException {
		...
		printerState.setSize(couponSize);
		...
		if (socket != null && output != null) {
			try {
				...
				// String convertCouponBarcodeString = couponLeaserPrintingList.stream()
				// .map(e -> e.toString().substring(2, e.toString().length()) + "|").reduce("|",
				// String::concat);
				// int listSize = couponLeaserPrintingList.size();
				...
				CouponProduction couponProduction = couponProductionService.getInfoByUuid(couponProdUuid);
				if (couponProduction != null) {
					couponProduction.setStatus(1);
				...
				while (true) {
					...
					String pendingCouponValues = null;

//					if (listSize > Integer.parseInt(arrOfCpn[0])) {
//
//						listSize = Integer.parseInt(arrOfCpn[0]);
//						for (String coupon : arrOfCpn)
//							if (coupon.length() > 10) {
//								pendingCouponValues += coupon + ",";
//							}
//
//						/*
//						 * List<CouponInspections> list =
//						 * couponInspectionsService.getByPrintingStatusChange(
//						 * pendingCouponValues.substring(0, pendingCouponValues.length() - 1)); for
//						 * (CouponInspections couponInspections : list) {
//						 * couponInspections.setStatus(2);
//						 * couponInspectionsService.update(couponInspections); }
//						 */
//					}
